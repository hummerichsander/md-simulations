...
print(
    "If you use this script in your work, please cite Pedersen et al. https://pubs.acs.org/doi/10.1021/acs.jctc.4c00553 DOI: 10.1021/acs.jctc.4c00553"
)
...
hbond_dict = {
    ...
    "HIS": {
        ...
        "SC2": [[0, 1], ["None", "IMD"]],
        "SC3": [[1, 0], ["IMA", "None"]],
    },  # assumed epsilon tautomer, but you should correct your pdb naming to HSE/HSD/HSP or HIE/HID/HIP naming (you should care about histidine tautomers)
    "HIE": {
        "BB": [[1, 1], ["AMA", "AMD"]],
        "SC1": [[0, 0], ["None", "None"]],
        "SC2": [[0, 1], ["None", "IMD"]],
...
special_residues = {
    ...
    "ASN",
    "ARG",
}  # HIS not included since it is mapped with higher resolution, allowing multiple HBs per side chain in the first matching round
...
energy_dict = {
    "IMA-AMD": 6.55 * 4.184,  # 4.184 converts from kcal/mol -> kJ/mol
    "IMA-HYD": 6.24 * 4.184,
    "IMA-IND": 7.11 * 4.184,
    "IMA-IMD": 7.96 * 4.184,
    "AMA-AMD": 5.44
    * 4.184,  # <-In the secondary structure model we exclude BB-BB (P2-P2) interactions, because the sigma for this interaction is larger than the structural distance between
    "AMA-HYD": 4.51
    * 4.184,  # most backbone beads in a proteins (one of the main reasons why the v3.0.0 mapping doesnt work for ss). To be consistent with the tertiary model that doesnt use exclusiosn
    "AMA-IND": 6.27
    * 4.184,  # we add back in the interaction energy of the P2-P2 LJ later in the function "combine and format potentials. The reason we dont use exclusion in the tertiary model
    "AMA-IMD": 6.92
    * 4.184,  # is that GROMACS doesnt support intermolecular exclusions and we want the tertiary model to work also for quaternary structure.
    "KEA-AMD": 4.06 * 4.184,
    "KEA-HYD": 3.54 * 4.184,
    "KEA-IND": 5.12 * 4.184,
    "KEA-IMD": 5.78 * 4.184,
    "PIA-AMD": 22.2329
    / 2,  # Divide by 2 because we split the pi interaction over the 2 outmost beads - important for the ring conformation
...
def user_input():
    ...
    parser.add_argument(
        "-c",
        help="File containing the coarse-grained structure(s) of the protein in pdb format. Multiple conformations can be supplied separated using comma: conf1.pdb,conf2.pdb,conf3.pdb. The conformations must have the exact same topology.",
    )
    parser.add_argument(
        "-i", help="File containing the protein topology e.g. molecule_0.itp from martinize2."
    )
    parser.add_argument(
        "--ss_cutoff",
        type=float,
        default=0.55,
        help="Cutoff distance for generation of the secondary network (and quaternary between BB beads) (default=0.55).",
    )
    parser.add_argument(
        "--ts_cutoff",
        type=float,
        default=0.55,
        help="Cutoff distance for generation of the tertiary network (and quaternary between non BB-BB pairs) (default=0.55).",
    )
    parser.add_argument(
        "--ss_scaling",
        type=float,
        default=1.0,
        help="Scaling of the secondary structure OLIVES bonds (default=1.0).",
    )
    parser.add_argument(
        "--ts_scaling",
        type=float,
        default=1.0,
        help="Scaling of the tertiary structure OLIVES bonds (default=1.0).",
    )
    parser.add_argument(
        "--qs_scaling",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="1.0",
        help='Multistate mode: Scaling of the unique contacts in each conformation, provided as a comma separated sting e.g. "0.5,0.75". Shared contacts between conformations have their distances averaged.',
    )
    parser.add_argument(
        "--extend_itp",
        type=int,
        default=True,
        help="Extend the protein topology with the Go-like model (default=True).",
    )
    parser.add_argument(
        "--write_separate_itp",
        type=bool,
        default=False,
        help="Writes the Go-like model as separate itp files to be included in the .top (default=False).",
    )
    parser.add_argument(
        "--write_vmd_itp",
        type=bool,
        default=False,
        help="Writes an itp with the Go-like bonds as harmonic bonds for visualization with VMD or use OLIVES as an elastic network (default=False).",
    )
    parser.add_argument(
        "--write_bond_information_file",
        type=bool,
        default=False,
        help="Writes a datafile with the bead indices for each contact pair. Does set analysis in the case of multiple conformations (default=False).",
    )
...
def check_topologies(pdbs_CG):
    ...
    if len(set(natoms)) > 1:
        raise ValueError(
            "Input conformations do not have the same number of beads. Please check your topology."
        )
    if len(set(nresidues)) > 1:
        raise ValueError(
            "Input conformations do not have the same number of residues. Please check your topology."
        )
    if len(set(nchains)) > 1:
        raise ValueError(
            "Input conformations do not have the same number of chains. Please check your topology."
        )


def verify_scaling_inputs(
    quaternary_energy_scaling, nchains, input_conformations, unique_pair_scaling
):
    ...
    if bool(quaternary_energy_scaling):
        ...
        if len(quaternary_energy_scaling) != expected_len:
            raise ValueError(
                "Incorrect number of quaternary structure scaling values. Expected {}.".format(
                    expected_len
                )
            )
    ...
    if not len(input_conformations) == len(unique_pair_scaling):
        raise ValueError(
            "The number of input conformations do not match the number of scaling values provided by the --unique_pair_scaling flag."
        )
...
def detect_cys_bridges(cut_pairs, cut_dists, a_ndx_to_res_name, a_ndx_to_bead_name):
    ...
    cys_bridges = [
        ...
        if a_ndx_to_res_name[pair[0]] == "CYS"
        and a_ndx_to_res_name[pair[1]] == "CYS"
        and (a_ndx_to_bead_name[pair[0]] != "BB" and a_ndx_to_bead_name[pair[1]] != "BB")
        and cut_dists[i] < 0.3
    ]
...
def should_ignore_cys(pair, cys_bridges, a_ndx_to_res_ndx):
    ...
    for b in cys_bridges:
        if np.array_equal(pair, b):
            return True
        if (
            a_ndx_to_res_ndx[pair[0]] == a_ndx_to_res_ndx[b[0]]
            and a_ndx_to_res_ndx[pair[1]] == a_ndx_to_res_ndx[b[1]]
        ):
            return True
        if (
            a_ndx_to_res_ndx[pair[0]] == a_ndx_to_res_ndx[b[1]]
            and a_ndx_to_res_ndx[pair[1]] == a_ndx_to_res_ndx[b[0]]
        ):
            return True
    return False
...
def process_pairs(cut_pairs, cut_dists, filters, cys_bridges):
    ...
    for i, pair in enumerate(cut_pairs):
        if a_ndx_to_bead_name[pair[0]] == "CA" or a_ndx_to_bead_name[pair[1]] == "CA":
            raise ValueError(
                "Atomistic structure detected or unexpected -govs-includes in martinize2."
            )
    ...
    return (
        np.array(pairs_secondary),
        np.array(dists_secondary),
        np.array(pairs_tertiary),
        np.array(dists_tertiary),
    )
...
def check_hb_pair(pair, dists, filters, hbond_dict, energy_dict, index):
    ...
    if hbond_lists[0][0][0] and hbond_lists[1][0][1]:  # First bead is HBA and second is HBD
        if (
            hbond_lists[0][0][1] and hbond_lists[1][0][0]
        ):  # First bead is also HBD and second is also HBA
...
def get_special_case_pairs(
    remaining_pairs, flat_matched_pairs, a_ndx_to_res_name, a_ndx_to_bead_name, special_residues
):
...
def get_pair_set_information(input_conformations, all_secondary_pairs, all_tertiary_pairs):
    ...
    if len(input_conformations) == 1:
        ...
        for c in conf_ndx:
            ...
            other_confs_set_secondary = set(
                item for sublist in other_confs_secondary for item in sublist
            )
            unique_pairs_secondary = conf_set_secondary - other_confs_set_secondary
            add_unique_pairs_info(
                info_secondary_pairs,
                input_conformations[c],
                unique_pairs_secondary,
                unique_secondary_pairs_for_scaling,
            )

            conf_set_tertiary = set(all_tertiary_pairs[c])
            other_confs_tertiary = [all_tertiary_pairs[i] for i in conf_ndx if i != c]
            other_confs_set_tertiary = set(
                item for sublist in other_confs_tertiary for item in sublist
            )
            unique_pairs_tertiary = conf_set_tertiary - other_confs_set_tertiary
            add_unique_pairs_info(
                info_tertiary_pairs,
                input_conformations[c],
                unique_pairs_tertiary,
                unique_tertiary_pairs_for_scaling,
            )
        ...
        for p, pair in enumerate(structure_pairs):
            ...
            if pair in current_entries:
                ...
                pairs_dict.update(
                    {
                        pair: currect_entry
                        + [[all_pairs_dists_energies[s][p][1], all_pairs_dists_energies[s][p][2]]]
                    }
                )
            else:
                pairs_dict.update(
                    {pair: [[all_pairs_dists_energies[s][p][1], all_pairs_dists_energies[s][p][2]]]}
                )
    return pairs_dict
...
def match_secondary_network(checked_pairs_dists_energies, checked_pairs):
    ...
    weighted_pairs_pass2 = create_weighted_pairs(
        [pair for pair in checked_pairs_dists_energies if pair[0] in remaining_pairs1]
    )
    ...
    weighted_pairs_pass3 = create_weighted_pairs(
        [pair for pair in checked_pairs_dists_energies if pair[0] in remaining_pairs2]
    )
    matched_pairs_pass3 = perform_max_weight_matching(weighted_pairs_pass3)

    matched_secondary_pairs = sorted(
        matched_pairs_pass1 + matched_pairs_pass2 + matched_pairs_pass3
    )
    matched_secondary_pairs_dists_energies = [
        [pair[0], pair[1], pair[2]]
        for pair in checked_pairs_dists_energies
        if pair[0] in matched_secondary_pairs
    ]
    return matched_secondary_pairs, matched_secondary_pairs_dists_energies


def match_tertiary_network(
    checked_pairs_dists_energies,
    checked_pairs,
    a_ndx_to_res_name,
    a_ndx_to_bead_name,
    ...
    special_case_pairs = get_special_case_pairs(
        remaining_pairs1,
        flat_matched_pairs_pass1,
        a_ndx_to_res_name,
        a_ndx_to_bead_name,
        special_residues,
    )
    ...
    weighted_pairs_pass2 = create_weighted_pairs(
        [pair for pair in checked_pairs_dists_energies if pair[0] in special_case_pairs]
    )
    matched_pairs_pass2 = perform_max_weight_matching(weighted_pairs_pass2)
    ...
    matched_tertiary_pairs_dists_energies = [
        [pair[0], pair[1], pair[2]]
        for pair in checked_pairs_dists_energies
        if pair[0] in matched_tertiary_pairs
    ]
    return matched_tertiary_pairs, matched_tertiary_pairs_dists_energies


def combine_and_format_potentials(
    pairs_dict,
    networktype,
    unique_pairs,
    unique_pair_scaling,
    ...
    for pair, dist_energy in pairs_dict.items():
        ...
        if not a_ndx_to_chain_ndx[pair[0]] == a_ndx_to_chain_ndx[pair[1]]:
            ...
            quaternary_pair_info.append(
                [str((pair[0] + 1, pair[1] + 1)), str((chain_one, chain_two))]
            )
        ...
        if len(unique_pair_scaling) > 1:
            for i, conf in enumerate(unique_pairs):
                if pair in conf:
                    HB_energy = (
                        np.array(dist_energy)[0, 1] * energy_scaling * unique_pair_scaling[i]
                    )
        ...
        mean_dist = np.mean(
            dists
        )  # Naive combination of minimas - ideally we would change the functional form, however we are very limited in GROMACS since tabulated potentials are currently unsupported
        ...
        harm_pairs.append(
            [
                str(pair[0] + 1),
                str(pair[1] + 1),
                f"{harm_function_type}",
                "{:.10f}".format(mean_dist),
                "{:.10f}".format(harm_k),
            ]
        )
        ...
        if networktype == "secondary":
            LJ_pairs.append(
                [
                    ...
                    str(pair[1] + 1),
                    f"{LJ_function_type}",
                    "{:.10f}".format(mean_dist / np.power(2, 1 / 6)),
                    "{:.10f}".format(HB_energy + 4.06),
                ]
            )  # Here we add back in the (unscaled) P2-P2 LJ energy that was exlcuded, to be consistent with the tertiary network
...
def generate_output(
    ...
    secondary_LJ_potentials,
    tertiary_LJ_potentials,
    secondary_harm_potentials,
    tertiary_harm_potentials,
    info_secondary_pairs,
    info_tertiary_pairs,
    secondary_quaternary_pair_info,
    tertiary_quaternary_pair_info,
    quaternary_energy_scaling,
):
    ...
    if args.extend_itp:
        ...
        append_to_itp(
            itp_CG,
            ["; OLIVES exclusions for secondary LJ 1-4 pairs".split()]
            + ["[ exclusions ]".split()]
            + secondary_exclusions
            + [[""]],
        )
        append_to_itp(
            itp_CG,
            ["; OLIVES secondary as LJ 1-4 pairs".split()]
            + ["[ pairs ]".split()]
            + secondary_LJ_potentials
            + [[""]],
        )
        append_to_itp(
            itp_CG, ["; OLIVES tertiary as LJ 1-4 pairs".split()] + tertiary_LJ_potentials + [[""]]
        )

    if args.write_separate_itp:
        print(
            f"If you use the --write_separate_itp True and --extend_itp False. Remember to include OLIVES itp's in your topology right after {itp_CG}"
        )
        print("Exclusion .itps should be above the interaction .itps")

        # Write the model as harmonic elastic network
        write_output(
            f"Exclusions_secondary_{itp_CG}",
            [[""]]
            + ["; OLIVES exclusions for secondary LJ 1-4 pairs".split()]
            + ["[ pairs ]".split()]
            + secondary_LJ_potentials
        ...
        write_output(
            f"LJ_tertiary_{itp_CG}",
            [[""]]
            + ["; OLIVES tertiary as LJ 1-4 pairs".split()]
            + ["[ pairs ]".split()]
            + tertiary_LJ_potentials
            + [[""]],
        )
        ...
        # Write the model as harmonic elastic network
        write_output(
            f"Harm_secondary_{itp_CG}",
            [[""]]
            + ["; OLIVES secondary as harmonic bonds".split()]
            + ["[ bonds ]".split()]
            + secondary_harm_potentials
            + [[""]],
        )
        write_output(
            f"Harm_tertiary_{itp_CG}",
            [[""]]
            + ["; OLIVES tertiary as harmonic bonds".split()]
            + ["[ bonds ]".split()]
            + tertiary_harm_potentials
            + [[""]],
        )
    ...
    if args.write_vmd_itp:
        ...
        write_vmd(
            f"OLIVES_VMD_secondary_{itp_CG}",
            pre_itp_lines,
            [[""]]
            + ["; OLIVES secondary as harmonic bonds".split()]
            + ["[ bonds ]".split()]
    ...
    if args.write_bond_information_file:
        ...
        if bool(quaternary_energy_scaling):
            write_output(
                "OLIVES_info_quaternary_pairs.dat",
                ["; OLIVES secondary pairs in quaternary bonds".split()]
                + ["; pair chainID".split()]
                + secondary_quaternary_pair_info
                + ["; OLIVES tertiary pairs in quaternary bonds".split()]
                + ["; pair chainID".split()]
                + tertiary_quaternary_pair_info,
            )
...
def main():
    ...
    secondary_energy_scaling = (
        args.ss_scaling
    )  # [kJ/mol] - Is multiplied with the relative hbond energies
    tertiary_energy_scaling = (
        args.ts_scaling
    )  # [kJ/mol] - Is multiplied with the relative hbond energies
    ...
    verify_scaling_inputs(
        quaternary_energy_scaling, pdbs_CG[0].n_chains, input_conformations, unique_pair_scaling
    )
    ...
    for p, pdb_CG in enumerate(pdbs_CG):
        ...
        all_pairs = np.array(
            [
                [i, j]
                for i in np.arange(pdb_CG.top.n_atoms)
                for j in np.arange(i + 1, pdb_CG.top.n_atoms)
            ]
        )
        all_dists = md.compute_distances(pdb_CG, all_pairs)[0]

        secondary_pairs, secondary_dists, tertiary_pairs, tertiary_dists = knowledge_based_checks(
            all_pairs, all_dists, filters
        )

        cut_secondary_pairs, cut_secondary_dists = apply_cutoff(
            secondary_pairs, secondary_dists, secondary_cutoff
        )
        cut_tertiary_pairs, cut_tertiary_dists = apply_cutoff(
            tertiary_pairs, tertiary_dists, tertiary_cutoff
        )

        # Check if HB pairs
        checked_secondary_pairs, checked_secondary_pairs_dists_energies = check_HB_pairs(
            cut_secondary_pairs, cut_secondary_dists, hbond_dict, energy_dict, filters
        )
        checked_tertiary_pairs, checked_tertiary_pairs_dists_energies = check_HB_pairs(
            cut_tertiary_pairs, cut_tertiary_dists, hbond_dict, energy_dict, filters
        )

        # Match networks
        matched_secondary_pairs, matched_secondary_pairs_dists_energies = match_secondary_network(
            checked_secondary_pairs_dists_energies, checked_secondary_pairs
        )
        matched_tertiary_pairs, matched_tertiary_pairs_dists_energies = match_tertiary_network(
            checked_tertiary_pairs_dists_energies,
            checked_tertiary_pairs,
            a_ndx_to_res_name,
            a_ndx_to_bead_name,
            special_residues,
        )
        ...
        if not args.silent:
            ...
            print(
                "Secondary HB pairs after weighted matching: {}".format(
                    len(matched_secondary_pairs)
                )
            )
            print(
                "Tertiary HB pairs after weighted matching: {}".format(len(matched_tertiary_pairs))
            )
    ...
    (
        ...
        unique_secondary_pairs_for_scaling,
        unique_tertiary_pairs_for_scaling,
    ) = get_pair_set_information(input_conformations, all_secondary_pairs, all_tertiary_pairs)

    # For each pair, check for if the pair occurs multiple times among all conformations
    secondary_pair_multiples_dict = construct_pair_multiples_dict(
        all_secondary_pairs, all_secondary_pairs_dists_energies
    )
    tertiary_pair_multiples_dict = construct_pair_multiples_dict(
        all_tertiary_pairs, all_tertiary_pairs_dists_energies
    )
    ...
    (
        ...
        secondary_exclusions,
        secondary_quaternary_pair_info,
    ) = combine_and_format_potentials(
        secondary_pair_multiples_dict,
        "secondary",
        unique_secondary_pairs_for_scaling,
        unique_pair_scaling,
        secondary_energy_scaling,
    ...
    if not args.silent:
        print(
            'Remember to use "gmx mdrun --noddcheck -rdd 2.0" to avoid domain decomposition warnings if OLIVES bonds gets too long.'
        )
        print(
        ...
        print(
            "This could happen if a protein complex dissociates or a protein unfolds. The 2 nm cutoff for domains in -rdd is where a LJ potential with energy minimum distance at 0.55 nm (the OLIVES cutoff) goes to 0, and therefore not important if missed."
        )
        print(
            "If you experience DD errors that cannot be solved by --noddcheck, try using 1 MPI tread and all openMP threads instead of domain decomposition https://manual.gromacs.org/documentation/current/user-guide/mdrun-performance.html."
        )
        print(
            "OLIVES has been tested using the -scfix flag of martinize2 - if you are using multistate OLIVES, remember to combine the -scfix flag with the OLIVES scfix script"
        )
        print(
            "OLIVES does not use the virtual site approach implemented in e.g. GoMARTINI and thus does not use the martinize2 flags -govs-include."
        )
        print(
            "OLIVES does not require -dssp secondary structure restraints, although one can opt to restrain the secondary structure at the cost of overstabilized secondary structure."
        )
